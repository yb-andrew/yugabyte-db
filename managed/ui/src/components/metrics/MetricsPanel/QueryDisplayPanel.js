...
export const QueryDisplayPanel = ({ universeUUID }) => {
  ...
  let highestMaxTime = 0;
  
  topQueries.forEach(val => {
    if (val.total_time > highestExecTime) {
      highestExecTime = val.total_time;
    }
    if (val.max_time > highestMaxTime) {
      highestMaxTime = val.max_time;
    }
  });
